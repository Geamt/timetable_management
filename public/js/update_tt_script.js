...
$(document).ready(function () {
    ...
    function loaddata() {
        $.ajax({
            url: "http://localhost:3000/scheduler/get_data_for_update",
            method: "post",
            data: {},
            success: function (data) {
                if (data['status'] == 200) {
                    //console.log(data['time_table']);
                    __time__table = data['time_table']['time_table'];
    ...
    $('.__table-body li').unbind().click(function () {
        ...
        $(document).on('change', '#faculty-list', function () {
            let $__last_changed_faculty = $('option:selected', this).attr('id');
            let $__same_subject = '';
                ...
                for (var div_key in divisions) {
                    for (var day_key in days) {
                        for (let j = 0; j < $('#lecture-time-list').children().length; j++) {
                            if (__time__table[sem_key][div_key][day_key][j] != undefined) {
                                //console.log(__time__table[sem_key][div_key][day_key][j]);
                                if (__time__table[sem_key][div_key][day_key][j].hasOwnProperty($__last_changed_faculty)) {
                                    ...
                                    //$__same_subject = key;
                                    $__same_subject = __time__table[sem_key][div_key][__json_day_index][$clicked_cell_index][key];
                                    console.log($__same_subject)
                                    break;
                                }
                            }
            ...
            console.log($__same_subject);
            $('#subject-list').val($__same_subject.subject);
    ...
    function displayFromJsonToHtml() {
        ...
        $('ul.__table-body').each(function () {
            ...
            $__table_body.children('li').each(function () {
                ...
                if ($__table_body_li.index() in __time__table[__json_sem_index][__json_div_index][__day_index]) {
    ...
    $('#btn_export').unbind().click(function () {     //Export the json array of current selected duration time table
        ...
        $.ajax({
            url: "http://localhost:3000/scheduler/update_time_table_data",
            method: "POST",
            data: { duration: duration, tt: JSON.stringify(time_table) },
            success: function (data) {
                if (data['status'] == 200) {
                    swal("Good job!", "Time table generated successfully !", "success").then(function() {
